...
f1 = h5py.File(dpath_hdf5+'label_blue_inter.hdf5','r') # readinf the labelled cube that contains inermediate values

labels_blue_inter = np.array(f1['label_3d_blue_inter'])

labels_in_roi =[]
for region in regionprops(labels_blue_inter):
...
for time in range(425): 
	reference_mask = np.where(labels_blue_inter[:,:,time]!=0,1.,0.)
...
def computing_pos_labels(labels_in_roi):
    ...
    time_indices = np.where(labels_blue_inter == labels_in_roi)
    unique_time_indices = np.unique(np.sort(time_indices[2]))
    position_per_time = []
    for time in range(len(unique_time_indices)):
        spatial_index=np.where(labels_blue_inter[:,:,unique_time_indices[time]] == labels_in_roi)
...
pool.close()
pool.join()
np.savez(dpath_cluster_fits+'rbe_COG_positions_per_label_inter',data)
