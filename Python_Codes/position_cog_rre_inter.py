...
f1 = h5py.File(dpath_hdf5+'label_red_inter.hdf5','r') # readinf the labelled cube that contains inermediate values

labels_red_inter = np.array(f1['label_3d_red_inter'])

labels_in_roi =[]
for region in regionprops(labels_red_inter):
...
for time in range(425): 
	reference_mask = np.where(labels_red_inter[:,:,time]!=0,1.,0.)
...
def computing_pos_labels(labels_in_roi):
    ...
    time_indices = np.where(labels_red_inter == labels_in_roi)
    unique_time_indices = np.unique(np.sort(time_indices[2]))
    position_per_time = []
    for time in range(len(unique_time_indices)):
        spatial_index=np.where(labels_red_inter[:,:,unique_time_indices[time]] == labels_in_roi)
...
pool.close()
pool.join()
np.savez(dpath_cluster_fits+'rre_COG_positions_per_label_inter',data)
