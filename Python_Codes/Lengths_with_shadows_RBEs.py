...
cluster_interest_blu =[5, 49,11, 25, 12, 48] ## Now with shadows as per shadow_movie.py
...
selem = diamond(1)
morph_processed_blue = master_aux_cube*0.
for scan in range(425):
    morph_processed_blue[:,:,scan] = binary_closing(binary_opening(master_aux_cube[:,:,scan],selem),selem) # Morph_open followed by Morph_closing operation to get rid of the salt
...
def compute_length_stats(label_number_3d):
    index = np.where(labeled_3d[0]==label_number_3d)
...
pool.close()
pool.join()
np.savez(dpath_cluster_fits+'rbe_stats_parallel_improved_shadow_RBEs',data)
