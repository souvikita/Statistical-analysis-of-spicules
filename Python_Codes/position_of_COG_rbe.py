...
def computing_pos_labels(labels_in_roi):
    ...
    time_indices = np.where(labeled_3d[0] == labels_in_roi)
    unique_time_indices = np.unique(np.sort(time_indices[2]))
    position_per_time = []
    for time in range(len(unique_time_indices)):
        spatial_index=np.where(labeled_3d[0][:,:,unique_time_indices[time]] == labels_in_roi)
...
pool.close()
pool.join()
np.savez(dpath_cluster_fits+'rbe_COG_positions_per_label',data)
