...
def compute_lifetime_parallel(label_number):
	...
	indices = np.where(label_3d_red_inter == label_number)
...
labeled_cube = h5py.File(dpath_cluster_fits+'label_red_inter.hdf5','r')
label_3d_red_inter = np.array(labeled_cube['label_3d_red_inter'])

label_nos_red_inter = []
for region in regionprops(label_3d_red_inter):
    label_nos_red_inter.append(region.label)

print("****Entering power parallel mode****")

labels_of_interest = label_nos_red_inter
pool = Pool(120)
