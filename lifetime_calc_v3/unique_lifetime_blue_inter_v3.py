...
def compute_lifetime_parallel(label_number):
	...
	indices = np.where(label_3d_blue_inter == label_number)
...
labeled_cube = h5py.File(dpath_cluster_fits+'label_blue_inter.hdf5','r')
label_3d_blue_inter = np.array(labeled_cube['label_3d_blue_inter'])

label_nos_blue_inter = []
for region in regionprops(label_3d_blue_inter):
    label_nos_blue_inter.append(region.label)

print("****Entering power parallel mode****")

labels_of_interest = label_nos_blue_inter
pool = Pool(120)
result = pool.map(compute_lifetime_parallel, labels_of_interest)
data = np.array(result)
pool.close()
pool.join()
np.savez(dpath_cluster_fits+'lifetime_blu_inter_uniq', data)
