...
def compute_lifetime_parallel(label_number):
	...
	indices = np.where(label_3d_blue_slow == label_number)
...
labeled_cube = h5py.File(dpath_cluster_fits+'label_blue_slow.hdf5','r')
label_3d_blue_slow = np.array(labeled_cube['label_3d_blue_slow'])

label_nos_blue_slow = []
for region in regionprops(label_3d_blue_slow):
    label_nos_blue_slow.append(region.label)

print("****Entering power parallel mode****")

labels_of_interest = label_nos_blue_slow
pool = Pool(60)
result = pool.map(compute_lifetime_parallel, labels_of_interest)
data = np.array(result)
pool.close()
pool.join()
np.savez(dpath_cluster_fits+'lifetime_blu_slow_uniq', data)
